...
- (instancetype)initWithRoundingRectImageView {
  self = [super init];
  if (self) {
    [self kl_cornerRadiusRoundingRect];
  }
  return self;
}
...
- (instancetype)initWithCornerRadiusAdvance:(CGFloat)cornerRadius rectCornerType:(UIRectCorner)rectCornerType {
  self = [super init];
  if (self) {
    [self kl_cornerRadiusAdvance:cornerRadius rectCornerType:rectCornerType];
  }
  return self;
}
 ...
 */
- (void)kl_cornerRadiusWithImage:(UIImage *)image cornerRadius:(CGFloat)cornerRadius rectCornerType:(UIRectCorner)rectCornerType {
 ...
 */
- (void)kl_cornerRadiusWithImage:(UIImage *)image cornerRadius:(CGFloat)cornerRadius rectCornerType:(UIRectCorner)rectCornerType backgroundColor:(UIColor *)backgroundColor {
  CGSize size = self.bounds.size;
  CGFloat scale = [UIScreen mainScreen].scale;
  CGSize cornerRadii = CGSizeMake(cornerRadius, cornerRadius);
...
- (void)kl_cornerRadiusAdvance:(CGFloat)cornerRadius rectCornerType:(UIRectCorner)rectCornerType {
  ...
  if (!self.hadAddObserver) {
    [[self class] swizzleMethod:NSSelectorFromString(@"dealloc") anotherMethod:@selector(kl_dealloc)];
...
- (void)validateFrame {
  if (self.frame.size.width == 0) {
    [self.class swizzleMethod:@selector(layoutSubviews) anotherMethod:@selector(kl_LayoutSubviews)];
  }
}
...
- (void)kl_LayoutSubviews {
  ...
  if (self.isRounding) {
    [self kl_cornerRadiusWithImage:self.image cornerRadius:self.frame.size.width/2 rectCornerType:UIRectCornerAllCorners];
  } else if (0 != self.radius && 0 != self.roundingCorners && nil != self.image) {
    [self kl_cornerRadiusWithImage:self.image cornerRadius:self.radius rectCornerType:self.roundingCorners];
  }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
  if ([keyPath isEqualToString:@"image"]) {
    ...
    [self validateFrame];
    if (self.isRounding) {
      [self kl_cornerRadiusWithImage:newImage cornerRadius:self.frame.size.width/2 rectCornerType:UIRectCornerAllCorners];
    } else if (0 != self.radius && 0 != self.roundingCorners && nil != self.image) {
      [self kl_cornerRadiusWithImage:newImage cornerRadius:self.radius rectCornerType:self.roundingCorners];
    }
  }
}
