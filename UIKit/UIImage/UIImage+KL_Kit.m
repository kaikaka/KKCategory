...
#import "UIImage+KL_Kit.h"
#import "CGRect+Additions.h"

@implementation UIImage (KL_Kit)
...
- (UIImage *)kl_drawText:(NSString *)text {
  return [self kl_drawText:text inRect:CGRectMake(0, 0, self.size.width, self.size.height)];
}

- (UIImage *)kl_drawText:(NSString *)text inRect:(CGRect)rect {
  return [self kl_drawText:text font:[UIFont systemFontOfSize:[UIFont labelFontSize]] inRect:rect];
}
...
- (NSData *)kl_data {
  return [self kl_dataWithCompressionQuality:.5];
}
...
- (UIImage *)kl_drawWithCornerRadius:(CGFloat)cornerRadius {
   ...
   */
  return [self kl_drawWithCornerRadius:cornerRadius size:self.size];
}

- (UIImage *)kl_drawWithCornerRadius:(CGFloat)cornerRadius size:(CGSize)size {
  return [self kl_drawWithCornerRadius:cornerRadius size:size borderWidth:.0 borderColor:nil];
}
